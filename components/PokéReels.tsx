...
import PokéReelsLogo from "./PokéReelsLogo"
...
export default function PokéReels() {
  ...
  const fetchPokemon = useCallback(async () => {
    ...
    const newPokemon = { ...data, ...speciesData }
    setPokemon((prev) => [...prev, newPokemon])
    setLoading(false)

    track("pokemon_viewed", { pokemon_id: newPokemon.id, pokemon_name: newPokemon.name })
  }, [])
  ...
  const handlePokédexClick = (pokemonName: string) => {
    track("pokédex_link_clicked", { pokemon_name: pokemonName })
  }
  ...
  return (
    <div className="h-screen overflow-y-auto snap-y snap-mandatory bg-gray-900 text-gray-100 relative">
      ...
      {pokemon.map((poke, index) => (
        ...
        >
          <div className="w-full h-2/5 relative bg-gray-800">
            <Image
          ...
          <div className="w-full h-3/5 bg-gray-900 p-4 flex flex-col overflow-y-auto">
            <div className="bg-gray-800 rounded-3xl p-4 shadow-lg flex-grow flex flex-col">
              <div className="flex justify-between items-center mb-2">
                <h2 className="text-2xl sm:text-3xl font-bold capitalize text-gray-100">{poke.name}</h2>
                <span className="text-xl font-semibold text-gray-400">#{poke.id}</span>
              </div>
              <div className="flex flex-wrap gap-2 mb-3">
                {poke.types.map((type) => (
                  <span
                    key={type.type.name}
                    className="px-2 py-1 rounded-full text-xs font-semibold text-gray-900"
              ...
              <div className="grid grid-cols-3 gap-2 mb-3 text-xs sm:text-sm text-gray-300">
              ...
              <div className="mb-3">
                <h3 className="text-sm sm:text-base font-semibold mb-2 text-gray-200">Base Stats:</h3>
              ...
              <a
                ...
                className="flex items-center justify-center gap-2 text-xs sm:text-sm bg-gray-700 hover:bg-gray-600 text-white py-2 px-4 rounded-full transition-colors duration-300 mt-auto"
                onClick={() => handlePokédexClick(poke.name)}
              >
